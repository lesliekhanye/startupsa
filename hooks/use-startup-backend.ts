...
export function useStartupBackend(){
  ...
  useEffect(()=>{const lifecycle=generation.current;let cancelled=false;let unsubscribe:(()=>void)|undefined;
    getSupabase().then(async c=>{if(cancelled)return;if(!c){setStatus('demo');return}setClient(c);
      const subscription=c.auth.onAuthStateChange((event,next)=>{if(cancelled||event==='INITIAL_SESSION')return;setSession(next);setModerator(false);setSubmissions([]);queueMicrotask(()=>{if(!cancelled)void refresh(c,next?.user.id)})});unsubscribe=()=>subscription.data.subscription.unsubscribe();
    ...
    return()=>{cancelled=true;lifecycle.value++;unsubscribe?.()};
  },[refresh]);
