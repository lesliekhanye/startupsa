...
export const publicStartup=cache(async(slug:string):Promise<StartupRecord|null>=>{
 ...
 const {data,error}=await client.from('startups').select('id,slug,name,website,pitch,story,category,city,stage,founded_year,founder,published_at,logo_path').eq('slug',slug).eq('hidden',false).maybeSingle();
