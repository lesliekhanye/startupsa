...
The canonical domain defaults to `https://startupsafrica.summit88.co.za`; operator/contact defaults are `startupsSA` and `central@summit88.co.za`. Public pages are open to search indexing; private routes remain excluded. PostHog can collect public-page views, selected actions, and masked session replays when `POSTHOG_PROJECT_KEY` is configured. `POSTHOG_HOST` defaults to the US ingestion host. PostHog uses memory persistence so analytics identity does not survive a page load; its temporary session ID groups replay events. Replay runs only on public pages, masks input values, and excludes sign-in dialogs. Automatic click capture remains disabled. This project currently builds as a Cloudflare Worker.
