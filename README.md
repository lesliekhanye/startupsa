...
The canonical domain defaults to `https://startups.summit88.co.za`; operator/contact defaults are `startupsSA` and `central@summit88.co.za`. Public pages are open to search indexing; private routes remain excluded. Vercel Analytics and Speed Insights load on public pages without a consent prompt; their endpoints must be provisioned through a suitable Vercel deployment/integration. This project currently builds as a Cloudflare Worker.
