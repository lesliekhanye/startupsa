   ...
   select jsonb_agg(
    (to_jsonb(s) - 'owner_id' - 'reviewed_by') || jsonb_build_object(
     ...
     'published_version',case when p.id is null then null else jsonb_build_object(
      'name',p.name,'website',p.website,'pitch',p.pitch,'story',p.story,
      'category',p.category,'city',p.city,'stage',p.stage,
      'founded_year',p.founded_year,'founder',p.founder,'logo_path',p.logo_path
     ) end
    )
