begin;
drop function public.edit_startup_submission(uuid,jsonb);
...
begin
 ...
 if coalesce(length(btrim(payload->>'name')),0) not between 2 and 80 or coalesce(length(btrim(payload->>'website')),0) not between 8 and 500 or btrim(payload->>'website') !~* '^https?://'
  ...
  or coalesce(length(btrim(payload->>'city')),0) not between 2 and 80 or payload->>'stage' not in ('Idea','Building','Launched','Revenue','Growing')
...
begin
 ...
 if decision='approved' then
  insert into public.startups(id,slug,name,website,pitch,story,category,city,stage,founded_year,founder,logo_path)
  values(item.id,trim(both '-' from regexp_replace(lower(item.name),'[^a-z0-9]+','-','g'))||'-'||left(item.id::text,8),item.name,item.website,item.pitch,item.story,item.category,item.city,item.stage,item.founded_year,item.founder,item.logo_path)
  on conflict(id) do update set name=excluded.name,website=excluded.website,pitch=excluded.pitch,story=excluded.story,category=excluded.category,city=excluded.city,stage=excluded.stage,founded_year=excluded.founded_year,founder=excluded.founder,logo_path=excluded.logo_path;
 end if;
