...
import {authenticate,boundedBody,deliverAdminReviewNotifications,deliverNotifications,response} from '@/lib/startup-server';
...
export async function POST(request:Request){
 ...
 try{
  ...
  const {error}=await auth.client.rpc('submit_startup',{payload:{...payload,logo_path:logoPath},request_id:id});if(error){console.error('Startup submission RPC failed',error.code,error.message);return response({error:process.env.NODE_ENV==='production'?'Could not save your submission. Please retry.':`Submission rejected: ${error.message}`},400)}
  const [delivery]=await Promise.all([deliverNotifications(auth.admin,auth.user.id,id),deliverAdminReviewNotifications(auth.admin)]);
  return response({id,emailPending:delivery.pending});
 }catch(e){console.error('Startup submission failed',e);return response({error:e instanceof z.ZodError?'Please check the submission details.':e instanceof Error&&['Invalid logo.','Upload is too large.','Logo upload failed. Please retry.'].includes(e.message)?e.message:process.env.NODE_ENV==='production'?'Could not save your submission. Check your details and logo, then retry.':`Could not save your submission: ${e instanceof Error?e.message:'Unknown error'}`},400)}
}
export async function PATCH(request:Request){
 ...
 try{
  ...
  await deliverAdminReviewNotifications(auth.admin);
