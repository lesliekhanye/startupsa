...
export async function POST(request:Request){
 ...
 try{
  ...
  const {data:moderator,error:roleError}=await auth.client.rpc('is_startup_moderator');if(roleError||!moderator)return response({error:'Administrator access is required.'},403);
  ...
  const {error}=await auth.client.rpc('review_startup',input);if(error)return response({error:'Could not review this submission. Check your moderator access and review details.'},400);
