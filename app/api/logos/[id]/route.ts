...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  const {data:published}=await admin.from('startups').select('logo_path').eq('id',id).eq('hidden',false).maybeSingle();
  let path=published?.logo_path;
  if(!published){const {client}=await authenticate(request);const {data:pending}=await client.from('startup_submissions').select('logo_path').eq('id',id).maybeSingle();path=pending?.logo_path}
