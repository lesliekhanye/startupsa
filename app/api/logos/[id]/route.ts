...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
 try{
  const id=z.string().uuid().parse((await params).id);const admin=adminClient();const version=new URL(request.url).searchParams.get('version');
  let path:string|null|undefined;
  if(version==='pending'){
   const {client}=await authenticate(request);const {data:pending}=await client.from('startup_submissions').select('logo_path').eq('id',id).maybeSingle();path=pending?.logo_path;
  }else if(version==='published'){
   const {data:published}=await admin.from('startups').select('logo_path').eq('id',id).eq('hidden',false).maybeSingle();path=published?.logo_path;
  }else{
